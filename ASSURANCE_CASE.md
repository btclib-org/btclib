...
**btclib and ellipticcurves.** The curve arithmetic and the schemes built
on it are ellipticcurves', a btclib-org package btclib binds again under
...
*The curve arithmetic is ellipticcurves'* describes.
...
own and ellipticcurves' alike, so whether anything crosses is decided by
...
**The environment.** `ELLIPTICCURVES_NO_LIBSECP256K1` is read once, by
ellipticcurves at import, and can only turn the delegation off.
...
- **Economy of mechanism.** One switch for the dispatch, as above, where
  ...
  `BTClibValueError` and ellipticcurves' `EllipticCurvesValueError` both
...
- **Weak randomness (CWE-330, CWE-338).** Randomness comes from `secrets`
  ...
  derived by RFC 6979 or by BIP340, each checked by ellipticcurves' suite
...
- **Improper verification of a signature (CWE-347).** A scheme whose
  ...
  ellipticcurves' suite for the schemes that package carries, and the
...
- **Exposure of sensitive information (CWE-209).** A refusal of octets
  ...
  the messages the key parsers refuse with, and ellipticcurves' own
