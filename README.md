...
Every workflow-status badge here carries `?branch=main`: unqualified, a
badge answers with the workflow's most recent run on any branch,
including a pull request's since-deleted one, which is issue #1467. A
sentinel that runs only on `schedule` has no run on `main` until its own
cron first fires, so its badge reads "no status" rather than a stale
result borrowed from elsewhere -- an accepted state, deliberately not
seeded with a manual `workflow_dispatch` run.
-->
