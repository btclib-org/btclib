...
from .network import (NETWORKS, curve_from_xpubversion,
                      network_from_xkeyversion, xpubversions_from_network)
...
def _point_from_xpub(xpub: BIP32Key, ec: Curve) -> Point:
    ...
    if xpub['key'][0] in (2, 3):
        ...
        if ec != ec2:
            raise ValueError(
                f"ec/xpub version ({xpub['version'].hex()}) mismatch")
...
def _pubkeyinfo_from_xpub(xpub: BIP32Key, network: Optional[str] = None,
...
def pubkeyinfo_from_key(key: Key, network: Optional[str] = None,
                        compressed: Optional[bool] = None) -> PubKeyInfo:
...
def pubkeyinfo_from_pubkey(P: PubKey, network: Optional[str] = None,
                           compressed: Optional[bool] = None) -> PubKeyInfo:
...
def pubkeyinfo_from_prvkey(prvkey: PrvKey, network: Optional[str] = None,
                           compressed: Optional[bool] = None) -> PubKeyInfo:
