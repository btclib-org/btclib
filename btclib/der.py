...
def to_bytes_min_length(n, byteorder="big"):
    ...
    # at least 1 byte, even if n = 0
    return n.to_bytes(n.bit_length() // 8 + 1, byteorder)
...
def check_DER_sig(DER_sig):
    ...
    assert 5 + \
        lenR <= len(
            DER_sig), "Length of the s element must be inside the signature"
    lenS = DER_sig[5 + lenR]
    ...
    if lenR > 1 and DER_sig[4] == 0x00:
        assert DER_sig[5] >= 0x80, \
    ...
    if lenS > 1 and DER_sig[lenR + 6] == 0x00:
        assert DER_sig[lenR + 7] >= 0x80, \
