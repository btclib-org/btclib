...
of the private key associated to a given address and, consequently, of the
...
def sign(prvkey: int, msg: str, compressed: bool) -> Tuple[str, str]:
    ...
    msg = _magic_msg(msg)
    sig = dsa.sign(secp256k1, sha256, msg, prvkey)

    pubkeys = dsa.pubkey_recovery(secp256k1, sha256, msg, sig)
...
def _verify(address: bytes, sig: bytes, msg: str) -> bool:
    ...
    sig = r, s
    pubkeys = dsa.pubkey_recovery(secp256k1, sha256, _magic_msg(msg), sig)

    return address == address_from_pubkey(pubkeys[i], compressed, b'\x00')
