...
def test_tx() -> None:
    ...
    assert not any(bool(tx_in.witness) for tx_in in tx.vin)
    ...
    assert any(bool(tx_in.witness) for tx_in in tx.vin)
    assert not tx.is_coinbase()
    assert tx.version == 1
    assert tx.lock_time == 0
    assert len(tx.vin) == 1
