#!/usr/bin/env python3

# Copyright (C) 2020 The btclib developers
#
# This file is part of btclib. It is subject to the license terms in the
# LICENSE file found in the top-level directory of this distribution.
#
# No part of btclib including this file, may be copied, modified, propagated,
# or distributed except according to the terms contained in the LICENSE file.

"Tests for the `btclib.witness` module."

import json
from os import path

import pytest

from btclib.exceptions import BTClibTypeError
from btclib.utils import bytes_from_octets
from btclib.witness import TxInWitness


def test_witness() -> None:
    witness = TxInWitness()
    assert witness.stack == []
    assert len(witness) == 0
    witness.assert_valid()
    assert witness == TxInWitness.deserialize(witness.serialize())

    stack = [
        "",
        "3044022077ecafa04bc23f87057bd54b572a473cf5cc6a945c167fcefe561618b86d097002200eb7c62a855295c8c288ff972c58905861a98260dafb6f8b7587cfda3091e00d01",
        "3045022100f20ba32865985e66985ba2d7ad11950309e253788b2edb27ccf5899e806f43ef02202899d98360f6476fcefbbf1dc1813595f4d0807d13237218de5ffaaeaa85640101",
        "304402206f7f5b0723d61f5d9f2ecbae448646d4cf0cf3ecade5ab50867fdfe8e131ce8f02207f0c260620e41dbdecd6ce574bf2773de248178d33f24aee71627843560137cd01",
        "01",
        "",
    ]
    witness = TxInWitness([bytes_from_octets(v) for v in stack])
    assert witness.stack == [bytes_from_octets(v) for v in stack]
    assert len(witness) == 6
    witness.assert_valid()
    assert witness == TxInWitness.deserialize(witness.serialize())


def test_exceptions() -> None:
    witness = TxInWitness("bad script", check_validity=False)  # type: ignore
    with pytest.raises(BTClibTypeError, match="invalid witness"):
        witness.assert_valid()

    witness = TxInWitness([b"", "", (32, 33)], check_validity=False)  # type: ignore
    with pytest.raises(BTClibTypeError, match="invalid witness"):
        witness.assert_valid()


def test_dataclasses_json_dict() -> None:
    witness_bytes = ""

    # dataclass
    witness_data = TxInWitness.deserialize(witness_bytes)
    assert isinstance(witness_data, TxInWitness)

    # TxInWitness to/from dict
    witness_dict = witness_data.to_dict()
    assert isinstance(witness_dict, dict)
    assert witness_data == TxInWitness.from_dict(witness_dict)

    datadir = path.join(path.dirname(__file__), "generated_files")

    # TxInWitness dict to/from dict file
    filename = path.join(datadir, "witness.json")
    with open(filename, "w") as file_:
        json.dump(witness_dict, file_, indent=4)
    with open(filename, "r") as file_:
        witness_dict2 = json.load(file_)
    assert isinstance(witness_dict2, dict)
    assert witness_dict == witness_dict2
