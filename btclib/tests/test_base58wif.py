...
from btclib.base58wif import (prvkeytuple_from_wif, prvkeytuple_from_xprvwif,
                              wif_from_prvkey, wif_from_xprv)
...
class TestWif(unittest.TestCase):
    ...
    def test_wif_from_prvkey(self):
        ...
        self.assertRaises(ValueError, prvkeytuple_from_wif, badwif)
        #prvkeytuple_from_wif(badwif)
    ...
    def test_prvkeytuple_from_xprvwif(self):
        ...
        # BIP32
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(xprv))
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(xprv_str))
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(' ' + xprv_str + ' '))
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(xprv_dict))

        # Invalid decoded size: 6 bytes instead of 82
        xpub = 'notakey'
        self.assertRaises(ValueError, prvkeytuple_from_xprvwif, xpub)
        #prvkeytuple_from_xprvwif(xpub)
        ...
        self.assertRaises(ValueError, prvkeytuple_from_xprvwif, xpub_dict)
        #prvkeytuple_from_xprvwif(xpub_dict)

        # WIF keys (bytes or string)
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(wif))
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(wif_str))
        self.assertEqual(ref_tuple, prvkeytuple_from_xprvwif(' ' + wif_str + ' '))
