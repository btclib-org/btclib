...
def test_native_p2wpkh():
    ...
    hash_ = sign_hash.from_prev_out(prev_out, tx, 1, sign_hash.ALL)
...
def test_native_p2wsh():
    ...
    hash_ = sign_hash.segwit_v0(script_, tx, 1, sign_hash.SINGLE, prev_out.value)
    assert hash_ == bytes.fromhex(
...
def test_wrapped_p2wsh():
    ...
    hash_ = sign_hash.from_prev_out(prev_out, tx, 0, sign_hash.NONE)
    assert hash_ == bytes.fromhex(
        "e9733bc60ea13c95c6527066bb975a2ff29a925e80aa14c213f686cbae5d2f36"
    )
    hash_ = sign_hash.from_prev_out(prev_out, tx, 0, sign_hash.SINGLE)
    assert hash_ == bytes.fromhex(
        "1e1f1c303dc025bd664acb72e583e933fae4cff9148bf78c157d1e8f78530aea"
    )
    hash_ = sign_hash.from_prev_out(
        prev_out, tx, 0, sign_hash.ANYONECANPAY | sign_hash.ALL
    )
    assert hash_ == bytes.fromhex(
        "2a67f03e63a6a422125878b40b82da593be8d4efaafe88ee528af6e5a9955c6e"
    )
    hash_ = sign_hash.from_prev_out(
        prev_out, tx, 0, sign_hash.ANYONECANPAY | sign_hash.NONE
    )
    assert hash_ == bytes.fromhex(
        "781ba15f3779d5542ce8ecb5c18716733a5ee42a6f51488ec96154934e2c890a"
    )
    hash_ = sign_hash.from_prev_out(
        prev_out, tx, 0, sign_hash.ANYONECANPAY | sign_hash.SINGLE
    )
    assert hash_ == bytes.fromhex(
