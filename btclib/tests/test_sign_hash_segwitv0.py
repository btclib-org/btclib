...
def test_native_p2wpkh():
    ...
    sign_hash = sign_hash_from_prev_out(previous_txout, tx, 1, ALL)
...
def test_native_p2wsh():
    ...
    tx.vin[1]._tx_in_witness = TxInWitness(
        [
            bytes.fromhex(
    ...
    script_code = _get_witness_v0_script_codes(tx.vin[1]._tx_in_witness.stack[-1])[1]
    sign_hash = segwit_v0(script_code, tx, 1, SINGLE, previous_txout.value)
    assert (
        sign_hash.hex()
...
def test_native_p2wsh_2():
    ...
    tx.vin[0]._tx_in_witness = TxInWitness(
        [
            bytes.fromhex(
    ...
    sign_hash = sign_hash_from_prev_out(previous_txout_1, tx, 0, ANYONECANPAY | SINGLE)
    assert (
        sign_hash.hex()
...
def test_wrapped_p2wsh():
    ...
    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, ALL).hex()
        == "185c0be5263dce5b4bb50a047973c1b6272bfbd0103a89444597dc40b248ee7c"
    )

    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, NONE).hex()
        == "e9733bc60ea13c95c6527066bb975a2ff29a925e80aa14c213f686cbae5d2f36"
    )

    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, SINGLE).hex()
        == "1e1f1c303dc025bd664acb72e583e933fae4cff9148bf78c157d1e8f78530aea"
    )

    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, ANYONECANPAY | ALL).hex()
        == "2a67f03e63a6a422125878b40b82da593be8d4efaafe88ee528af6e5a9955c6e"
    )

    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, ANYONECANPAY | NONE).hex()
        == "781ba15f3779d5542ce8ecb5c18716733a5ee42a6f51488ec96154934e2c890a"
    )

    assert (
        sign_hash_from_prev_out(previous_txout, tx, 0, ANYONECANPAY | SINGLE).hex()
