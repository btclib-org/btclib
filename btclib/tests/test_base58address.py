...
class TestAddresses(unittest.TestCase):
    ...
    def test_p2sh(self):
        ...
        assert script_bytes.hex() == "6e879169a77ca787"
        ...
        assert is_p2sh
        assert redeem_script_hash == hash160(script_bytes)
    ...
    def test_address_from_wif(self):
        ...
        b58 = p2pkh(wif1)
        assert b58 == b"1LPM8SZ4RQDMZymUmVSiSSvrDfj1UZY9ig"
        self.assertRaises(ValueError, p2wpkh, wif1)
        self.assertRaises(ValueError, p2wpkh_p2sh, wif1)

        # compressed mainnet
        wif2 = "Kx621phdUCp6sgEXPSHwhDTrmHeUVrMkm6T95ycJyjyxbDXkr162"
        b58 = p2pkh(wif2)
        assert b58 == b"1HJC7kFvXHepkSzdc8RX6khQKkAyntdfkB"
        b32 = p2wpkh(wif2)
        assert h160_from_b58address(b58)[1:] == witness_from_b32address(b32)[1:]
        h160 = h160_from_b58address(b58)[1]
        b = p2wpkh_p2sh(wif2)
        assert hash160(b"\x00\x14" + h160) == h160_from_b58address(b)[1]

        assert prvkeyinfo_from_prvkey(wif1)[0] == prvkeyinfo_from_prvkey(wif2)[0]

        # uncompressed testnet
        wif1 = "91gGn1HgSap6CbU12F6z3pJri26xzp7Ay1VW6NHCoEayNXwRpu2"
        b58 = p2pkh(wif1)
        assert b58 == b"mvgbzkCSgKbYgaeG38auUzR7otscEGi8U7"
        self.assertRaises(ValueError, p2wpkh, wif1)
        self.assertRaises(ValueError, p2wpkh_p2sh, wif1)
        ...
        b58 = p2pkh(wif2)
        assert b58 == b"n1KSZGmQgB8iSZqv6UVhGkCGUbEdw8Lm3Q"
        b32 = p2wpkh(wif2)
        assert h160_from_b58address(b58)[1:] == witness_from_b32address(b32)[1:]
        h160 = h160_from_b58address(b58)[1]
        b = p2wpkh_p2sh(wif2)
