...
def _get_witness_sighash(
    ...
    if sighash_type == 1 or sighash_type == 80:
        hashOutputs = b""
        for vout in transaction["vout"]:
            hashOutputs += tx_out.serialize(vout)
        hashOutputs = hash256(hashOutputs)
    elif sighash_type == 3 and input_index < len(transaction["vout"]):
        hashOutputs = hash256(tx_out.serialize(transaction["vout"][input_index]))
    else:
    ...
    sighash_type = (sighash_type).to_bytes(4, "little")
...
def _get_witness_scriptCodes(scriptPubKey):
    if scriptPubKey[0] == 0:  # simple p2wpkh
...
def get_sighash(
    transaction: tx.Tx, previous_output: tx.TxOut, input_index: int, sighash_type: int
) -> bytes:
