...
def jac_from_affine(Q: Optional[Point]) -> JacPoint:
    if Q is None:
        return (1, 1, 0)
    assert (isinstance(Q, tuple) and len(Q) == 2),  "point not in affine coordinates"
    return (Q[0], Q[1], 1)
...
class EllipticCurve:
    ...
    def yQuadraticResidue(self, x: int, quadRes: int) -> int:
        ...
        if quadRes:
            return self._p - root if (self.jacobi(root) != 1) else root
        else:
            return root if (self.jacobi(root) != 1) else self._p - root
