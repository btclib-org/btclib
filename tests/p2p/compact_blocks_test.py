...
def test_a_memoryview_wtxid_gives_the_same_short_id_as_bytes() -> None:
    """`short_id` reverses the wtxid it is given before hashing it.

    `[::-1]` over a memoryview is a strided, non-contiguous view, which
    `siphash`'s own call to `bytes_from_octets` refuses: a wtxid spelled
    as a memoryview would raise where every other `Octets` spelling
    answers (issue #1429).
    """
    block = _block_481824()
    compact_block = _compact(block)
