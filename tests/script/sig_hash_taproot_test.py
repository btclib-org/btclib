...
def test_invalid_taproot_key_path(vector: dict[str, Any]) -> None:
    ...
    # btclib's class or, from `ssa` itself, in ellipticcurves'. Neither
...
def test_key_path_spend_round_trip(hash_type: int, script_tree: Any) -> None:
    ...
    # and ellipticcurves says "signature verification failed", while the
    # Python arm knows which of BIP340's checks failed and says so -- "y_K
    # is odd" for these signatures. Both are EllipticCurvesRuntimeError,
    # `ssa` being ellipticcurves', which is the contract, and dropping the
    ...
    with pytest.raises(EllipticCurvesRuntimeError, match=_REFUSED):
