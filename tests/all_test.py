...
import ellipticcurves.ecc
import ellipticcurves.ecc.ellswift
import ellipticcurves.exceptions
import ellipticcurves.hashes
import pytest
...
#
# The `ellipticcurves` package is the third decision (issue #2282): the
...
def _the_whole_of(dotted: str) -> tuple[ModuleType, list[str]]:
    """Return an ellipticcurves module and every name it publishes."""
    canonical = import_module(f"ellipticcurves.{dotted}")
...
REEXPORTED = {
    ...
    "btclib.hashes": _reexports(
        (ellipticcurves.hashes, ["reduce_to_hlen", "tagged_hash"]),
    ),
    ...
    "btclib.ecc.ellswift": _reexports(
        (ellipticcurves.ecc.ellswift, ["create_var", "decode_var", "encode_var"]),
    ),
    "btclib.curves": _reexports(_the_whole_of("curves")),
    "btclib.ecc": _reexports(
        (ellipticcurves.ecc, ["diffie_hellman", "second_generator"]),
    ),
    **{
        f"btclib.{dotted}": _reexports(_the_whole_of(dotted))
        for dotted in _ELLIPTICCURVES_MODULES
    },
}
...
def test_ec_exports_the_curve_api_not_the_benchmark() -> None:
    ...
    strength of its name -- gives up. The variants are ellipticcurves'
...
def test_no_module_exports_a_name_it_imported() -> None:
    ...
    modules btclib keeps for the `ellipticcurves` package bind that
