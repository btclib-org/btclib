...
def test_signature() -> None:
    ...
    addr = b58.p2pkh(b58.prv_key_data_from_wif(wif).pub)
    bms_sig = bms.sign(msg, b58.prv_key_data_from_wif(wif))
    bms.assert_as_valid(msg, addr, bms_sig)
    assert bms.verify(msg, addr, bms_sig)
...
def test_exceptions() -> None:
    ...
    b58_p2pkh = b58.p2pkh(b58.prv_key_data_from_wif(wif).pub)
    b32_p2wpkh = b32.p2wpkh(b58.prv_key_data_from_wif(wif).pub)
    b58_p2wpkh_p2sh = b58.p2wpkh_p2sh(b58.prv_key_data_from_wif(wif).pub)
...
def test_one_prv_key_multiple_addresses() -> None:
    ...
    # Compressed WIF
    wif = "Kx45GeUBSMPReYQwgXiKhG9FzNXrnCeutJp4yjTd5kKxCitadm3C"
    b58_p2pkh_compressed = b58.p2pkh(b58.prv_key_data_from_wif(wif).pub)
    b58_p2wpkh_p2sh = b58.p2wpkh_p2sh(b58.prv_key_data_from_wif(wif).pub)
    b32_p2wpkh = b32.p2wpkh(b58.prv_key_data_from_wif(wif).pub)
    ...
    b58_p2pkh_uncompressed = b58.p2pkh(b58.prv_key_data_from_wif(wif2).pub)
    ...
    b58_p2pkh_compressed = b58.p2pkh(b58.prv_key_data_from_wif(wif3).pub)
    b58_p2wpkh_p2sh = b58.p2wpkh_p2sh(b58.prv_key_data_from_wif(wif3).pub)
    b32_p2wpkh = b32.p2wpkh(b58.prv_key_data_from_wif(wif3).pub)
...
def test_msgsign_p2pkh() -> None:
    ...
    add1u = b58.p2pkh(b58.prv_key_data_from_wif(wif1u).pub)
    ...
    add1c = b58.p2pkh(b58.prv_key_data_from_wif(wif1c).pub)
...
def test_ledger() -> None:
    ...
    The two keys are what BIP32 derives at `m/1` and `m/0/0` from the
    BIP39 seed of the mnemonic "barely sun snack this snack relief pipe
    attack disease boss enlist lawsuit", with no passphrase: the
    derivation is `btclib_wallet`'s, so the scalars are written out.
    """
    # non-standard leading 31 in DER serialization
    q = 0x4325CDB483C8CD28059BF35F2F73A2F89B7B0F46A11E595E7E5064C453EAC994
    ...
    bms.sign(msg, PrvKeyData(q))

    # standard leading 30 in DER serialization
    q = 0x33469A372F8F8418FAD79D3210EBBDF4EAE6828FCEE8898A0E1D4B3DC1F527FB
...
def _recovers(key_id: int, magic_msg: bytes, dsa_sig: dsa.Sig) -> Point | None:
    # btclib_ecc's two classes, `dsa` being that package's
...
def test_a_key_id_that_recovers_nothing() -> None:
    ...
    for key_id in (2, 3):
        ...
        # verify, which is the RuntimeError -- each btclib_ecc's class
...
def test_the_py_arm_reaches_no_bindings(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    reach, module and already-bound name alike, btclib_ecc's bindings
