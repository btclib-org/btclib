...
ellipticcurves asks for the bindings once, at import, and btclib's own
...
from btclib.exceptions import BTClibException, EllipticCurvesException
...
# them: ellipticcurves' own, read once, when its dispatch is first imported
_NO_LIBSECP256K1 = "ELLIPTICCURVES_NO_LIBSECP256K1"
...
def test_the_environment_variable_refuses_the_installed_bindings() -> None:
    ...
    It is ellipticcurves', and what this asks is that btclib answers to
...
def refused(call):
    try:
        call()
    except (BTClibException, EllipticCurvesException) as e:
...
def _locally_refused(call: Callable[[], object]) -> tuple[str, str]:
    ...
    with pytest.raises((BTClibException, EllipticCurvesException)) as excinfo:
        call()
