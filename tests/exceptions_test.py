...
from tests import defined_by_ellipticcurves
...
def test_every_exception_of_the_module_is_one_base_to_catch() -> None:
    ...
    `except` names. The classes ellipticcurves defines, bound here again,
    ...
    for cls in classes:
        if cls is BTClibUserWarning:
            ...
            continue
        if defined_by_ellipticcurves(cls):
            assert issubclass(cls, EllipticCurvesException), cls.__name__
...
def test_a_failure_raised_inside_ellipticcurves_is_caught_by_the_builtin() -> None:
    """`except ValueError` catches it, and `except BTClibException` does not.

    What a btclib name bound again from ellipticcurves raises is that
    ...
    with ellipticcurves' `scalar_from_prv_key`. The built-in is the one
    ...
    for raised in (parsed.value, read.value):
        assert isinstance(raised, EllipticCurvesException)
