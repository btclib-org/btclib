...
# has to be refused as one of btclib's three classes, or of ellipticcurves'
...
_CASES: list[tuple[str, type[Any], bytes]] = [
    ...
    ("p2p_getaddr", GetAddr, GetAddr().serialize()),
    ("p2p_mempool", Mempool, Mempool().serialize()),
    ("p2p_sendheaders", SendHeaders, SendHeaders().serialize()),
    ("p2p_wtxidrelay", WtxidRelay, WtxidRelay().serialize()),
    ("p2p_sendtxrcncl", SendTxRcncl, SendTxRcncl(1, 2).serialize()),
    ("p2p_feefilter", FeeFilter, FeeFilter(1000).serialize()),
    ("p2p_feature", Feature, Feature(b"BIP434", b"\x01").serialize()),
    ...
    (
        "p2p_getcfilters",
        GetCFilters,
        GetCFilters(BlockFilterType.BASIC, 1, _inventory().hash).serialize(),
    ),
    (
        "p2p_cfilter",
        CFilter,
        CFilter(BlockFilterType.BASIC, _inventory().hash, b"\x01\x02").serialize(),
    ),
    (
        "p2p_getcfheaders",
        GetCFHeaders,
        GetCFHeaders(BlockFilterType.BASIC, 1, _inventory().hash).serialize(),
    ),
    (
        "p2p_cfheaders",
        CFHeaders,
    ...
    (
        "p2p_getcfcheckpt",
        GetCFCheckpt,
        GetCFCheckpt(BlockFilterType.BASIC, _inventory().hash).serialize(),
    ),
    (
        "p2p_cfcheckpt",
        CFCheckpt,
        CFCheckpt(BlockFilterType.BASIC, _inventory().hash, [bytes(32)]).serialize(),
    ),
    ("p2p_sendcmpct", SendCmpct, SendCmpct(announce=True).serialize()),
    (
        "p2p_prefilled_transaction",
        PrefilledTransaction,
