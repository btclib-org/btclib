...
# --------------------------------------------------------------------------
# The spellings of one key pair, read by `ecc/dsa_test.py` and
# `hashes_test.py`. Both ask the same kind of question of a public API --
# which spellings it takes and which it refuses -- so the scalar, its hex,
# the point and the two SEC forms are built here rather than twice.
#
