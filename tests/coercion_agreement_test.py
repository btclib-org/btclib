...
"""btclib's input coercions and ellipticcurves' copies of them agree.

A value a caller hands to btclib may be read by either package: by
`btclib.utils` where btclib reads it, by `ellipticcurves._utils` where a
name bound again from ellipticcurves does (issue #2282). Two copies of one
...
built-in family: `BTClibValueError` against `EllipticCurvesValueError`, and
so on, the two packages' bases being different by design.

`ellipticcurves._utils` is private, and this test reaches it anyway: its
subject is the agreement of the two copies, and publishing the coercions
would make them ellipticcurves API for a test's sake.
"""
...
from ellipticcurves import _utils as ellipticcurves_utils
...
def test_both_copies_answer_alike(
    ...
    theirs = _outcome(getattr(ellipticcurves_utils, name), args())
    assert ours == theirs
...
def test_every_shared_coercion_has_a_vector() -> None:
    ...
    shared = {
        ...
        if isfunction(getattr(btclib_utils, n)) and hasattr(ellipticcurves_utils, n)
    }
