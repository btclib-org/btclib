...
from btclib.exceptions import BTClibTypeError, EllipticCurvesTypeError
...
_CASES: list[tuple[str, Callable[[Any], object]]] = [
    ...
    # reaches it through ellipticcurves' copy, which is that package's to
...
def test_a_bool_is_not_an_integer_field(
    ...
    ellipticcurves' class where that package reads the field -- the WIF's
...
def test_which_check_refuses_the_bool_decides_the_sentence(
    ...
    with pytest.raises((BTClibTypeError, EllipticCurvesTypeError), match=message):
        call(True)
