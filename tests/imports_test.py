...
def test_the_codec_does_not_pay_for_the_rpc_package() -> None:
    ...
    `urllib.request`, `ssl` and `socket` in with it, none of which a
    codec has any use for. README.md states the property this keeps: "No
    module loads `urllib.request` on its way to anything else".
    ...
    """
    socket_machinery = ("'urllib.request'", "'ssl'", "'socket'", "'bitcoin_core_rpc'")

    probe = "import btclib.p2p, sys; print(sorted(sys.modules))"
    loaded = subprocess.run(  # noqa: S603
        [sys.executable, "-c", probe], check=True, capture_output=True, encoding="utf-8"
    ).stdout
    for name in socket_machinery:
