...
- **`except BTClibException` does not catch what the curve arithmetic and
  ...
  `bms` are ellipticcurves' objects bound again, and they raise that
  package's `EllipticCurvesValueError`, `EllipticCurvesTypeError` and
  `EllipticCurvesRuntimeError`, as does a btclib function handing its
  argument on to them: `b58.wif_from_prv_key` refuses a key out of range
  with `EllipticCurvesValueError`. v2023.7.12's `dsa.Sig.parse` raised
  `BTClibValueError`. Each class derives from the built-in its name ends
  in, so act on it by catching `ValueError`, `TypeError` or
  `RuntimeError`, or by naming `EllipticCurvesException`, which
...
- **`BTCLIB_NO_LIBSECP256K1` is `ELLIPTICCURVES_NO_LIBSECP256K1`** (issue
  #2282): ellipticcurves reads the variable that starts a process with
