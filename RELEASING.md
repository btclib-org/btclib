...
1. Run `uv run pre-commit run --all-files` and `uv run pytest --cov`,
   ...
   Naming `--subject` and `--body-file` explicitly is harmless at any commit
   count — it simply replaces the two `squash_merge_commit_*` defaults with a
   title and body chosen on purpose. The commit that lands under the tag is one
   GitHub composes at the button: a squash always mints a fresh commit object,
   so no local commit is ever what ends up on `main`, whatever the branch held.
   That is why the checks are read again below, on what `main` ends up at
