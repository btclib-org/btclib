...
**Whose they are.** Every scheme but bms is the `ellipticcurves`
package's, `ellipticcurves.ecc`, which btclib depends on: each module
here other than `bms` binds that package's objects again under the
spelling a caller already had, `btclib.ecc.dsa.sign` being
`ellipticcurves.ecc.dsa.sign` (issue #2282). `ellswift` adds what BIP324
...
from ellipticcurves.ecc import diffie_hellman, second_generator
