...
from ECsecp256k1 import pointAdd, pointMultiply, \
                        order as ec_order, prime as ec_prime, \
                        a as ec_a, b as ec_b
...
def decode_prv(prv):
  ...
  if type(prv) == str:
    if prv[:2] == "0x":
      prv = prv[2:].zfill(64)
    if all(c in hexdigits for c in prv) and len(prv) == 64:
      prv = int(prv, 16)  # hex
    elif all(c in b58digits for c in prv):  # Wif
      prv = b58decode_check(prv)[1:]
      if len(prv) == 33:  # compressed
        prv = prv[:-1]
      # may it happen that a str may represent both a wif and a hex?
      # should check in a better way, e.g. wif starts with a digit
    else:
      assert 0, "if private key is a string, it must be hex or Wif"
  if type(prv) == bytes:
...
if __name__ == "__main__":
  ...
  prv_from_wif1 = decode_prv("5HueCGU8rMjxEXxiPuD5BDku4MkFqeZyd4dZ1jvhTVqvbTLvyTJ")  # uncompressed
  prv_from_wif2 = decode_prv("KwdMAjGmerYanjeui5SHS7JkmpZvVipYvB2LJGU1ZxJwYvP98617")  # compressed
  prv_from_hex = decode_prv(hex(prv))
  prv_from_bytes = decode_prv(prv.to_bytes(32, "big"))
  assert prv_from_wif1 == prv and \
         prv_from_wif2 == prv and \
         prv_from_hex == prv and \
         prv_from_bytes == prv

  pub = (0xD0DE0AAEAEFAD02B8BDC8A01A1B8B11C696BD3D66A2C5F10780D95B7DF42645C,
         0xD85228A6FB29940E858E7E55842AE2BD115D1ED7CC0E82D934E929C97648CB0A)
  pub_from_ext_hex = decode_pub("04" + hex(pub[0])[2:] + hex(pub[1])[2:])
  pub_from_com_hex = decode_pub("02" + hex(pub[0])[2:])
  pub_from_ext_bytes = decode_pub(b'\x04' + pub[0].to_bytes(32, "big") + pub[1].to_bytes(32, "big"))
  pub_from_com_bytes = decode_pub(b'\x02' + pub[0].to_bytes(32, "big"))
  assert pub_from_ext_hex == pub and \
         pub_from_com_hex == pub and \
         pub_from_ext_bytes == pub and \
         pub_from_com_bytes == pub
